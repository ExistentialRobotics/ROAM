...
def oriented_astar(goal, start, occupancy_map, footprint, obstacle_values, planning_resolution=None, delta=0.0, epsilon=1.0, allow_diagonal=True):
    """
    ...
    :param start array(3)[float]: [x, y, theta] start pose of the robot
    :param occupancy_map Costmap: occupancy map used for planning, data must be compatible with uint8
    :param footprint CustomFootprint: footprint of the robot used for collision checking
    :param obstacle_values array(N)[uint8]: an array containing values that the collision checker should deem as an obstacle
                             i.e [127, 0]
    :param planning_resolution float: a lower resolution than the original occupancy map resolution to plan on.
    :param delta float: distance in pixels to extend the goal region for solution (allow to be within delta away from goal)
                  TODO FIX this to be in meters
    :param epsilon float: weighting for the heuristic in the A* algorithm
    :param allow_diagonal bool: whether to allow diagonal movements
    ...
    footprint_masks = footprint.get_footprint_masks(occupancy_map.resolution, angles=c_angles)
    outline_coords = footprint.get_outline_coords(occupancy_map.resolution, angles=c_angles)
    ...
    if planning_resolution is not None:
        assert occupancy_map.resolution <= planning_resolution
        planning_scale = int(np.round(planning_resolution / occupancy_map.resolution))
    else:
        planning_scale = 1
